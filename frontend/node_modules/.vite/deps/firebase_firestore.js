...
function __PRIVATE_logDebug(e, ...t) {
  if (M.logLevel <= LogLevel.DEBUG) {
    ...
    M.debug(`Firestore (${F}): ${e}`, ...n);
  }
}
function __PRIVATE_logError(e, ...t) {
  if (M.logLevel <= LogLevel.ERROR) {
    const n = t.map(__PRIVATE_argToString);
    M.error(`Firestore (${F}): ${e}`, ...n);
  }
}
function __PRIVATE_logWarn(e, ...t) {
  if (M.logLevel <= LogLevel.WARN) {
    const n = t.map(__PRIVATE_argToString);
    M.warn(`Firestore (${F}): ${e}`, ...n);
  }
}
...
function __PRIVATE__fail(e, t, n) {
  let r = `FIRESTORE (${F}) INTERNAL ASSERTION FAILED: ${t} (ID: ${e.toString(16)})`;
  if (void 0 !== n) try {
...
var Timestamp = class _Timestamp {
   ...
   */
  constructor(e, t) {
    if (this.seconds = e, this.nanoseconds = t, t < 0) throw new FirestoreError(x.INVALID_ARGUMENT, "Timestamp nanoseconds out of range: " + t);
    if (t >= 1e9) throw new FirestoreError(x.INVALID_ARGUMENT, "Timestamp nanoseconds out of range: " + t);
    if (e < O) throw new FirestoreError(x.INVALID_ARGUMENT, "Timestamp seconds out of range: " + e);
    if (e >= 253402300800) throw new FirestoreError(x.INVALID_ARGUMENT, "Timestamp seconds out of range: " + e);
  }
  /**
...
var ResourcePath = class _ResourcePath extends BasePath {
  ...
  static fromString(...e) {
    const t = [];
    for (const n of e) {
      if (n.indexOf("//") >= 0) throw new FirestoreError(x.INVALID_ARGUMENT, `Invalid segment (${n}). Paths must not contain // in them.`);
...
var FieldPath$1 = class _FieldPath$1 extends BasePath {
  ...
  static fromServerFormat(e) {
    ...
    const __PRIVATE_addCurrentSegment = () => {
      if (0 === n.length) throw new FirestoreError(x.INVALID_ARGUMENT, `Invalid field path (${e}). Paths must not be empty, begin with '.', end with '.', or contain '..'`);
      t.push(n), n = "";
    };
    let i = false;
    for (; r < e.length; ) {
      const t2 = e[r];
      if ("\\" === t2) {
        if (r + 1 === e.length) throw new FirestoreError(x.INVALID_ARGUMENT, "Path has trailing escape character: " + e);
        const t3 = e[r + 1];
        if ("\\" !== t3 && "." !== t3 && "`" !== t3) throw new FirestoreError(x.INVALID_ARGUMENT, "Path has invalid escape sequence: " + e);
        n += t3, r += 2;
      } else "`" === t2 ? (i = !i, r++) : "." !== t2 || i ? (n += t2, r++) : (__PRIVATE_addCurrentSegment(), r++);
    }
    if (__PRIVATE_addCurrentSegment(), i) throw new FirestoreError(x.INVALID_ARGUMENT, "Unterminated ` in path: " + e);
...
var IndexOffset = class _IndexOffset {
  ...
  static min() {
    return new _IndexOffset(SnapshotVersion.min(), DocumentKey.empty(), k);
  }
  /** Returns an offset that sorts after all regular offsets. */
  static max() {
    return new _IndexOffset(SnapshotVersion.max(), DocumentKey.empty(), k);
  }
};
...
async function __PRIVATE_ignoreIfPrimaryLeaseLoss(e) {
  if (e.code !== x.FAILED_PRECONDITION || e.message !== q) throw e;
...
var __PRIVATE_SimpleDbTransaction = class ___PRIVATE_SimpleDbTransaction {
  ...
  abort(e) {
    e && this.S.reject(e), this.aborted || (__PRIVATE_logDebug(Q, "Aborting transaction:", e ? e.message : "Client-initiated abort"), this.aborted = true, this.transaction.abort());
  }
  v() {
  ...
  async k(e) {
    return this.db || (__PRIVATE_logDebug(Q, "Opening database:", this.name), this.db = await new Promise((t, n) => {
      ...
      r.onsuccess = (e2) => {
        ...
        "VersionError" === r2.name ? n(new FirestoreError(x.FAILED_PRECONDITION, "A newer version of the Firestore SDK was previously used and so the persisted data is not compatible with the version of the SDK you are now using. The SDK will operate with persistence disabled. If you need persistence, please re-upgrade to a newer version of the SDK or else clear the persisted IndexedDB data for your app to start fresh.")) : "InvalidStateError" === r2.name ? n(new FirestoreError(x.FAILED_PRECONDITION, "Unable to open an IndexedDB connection. This could be due to running in a private browsing session on a browser whose private browsing sessions do not support IndexedDB: " + r2)) : n(new __PRIVATE_IndexedDbTransactionError(e, r2));
      }, r.onupgradeneeded = (e2) => {
        __PRIVATE_logDebug(Q, 'Database "' + this.name + '" requires upgrade from version:', e2.oldVersion);
  ...
  async runTransaction(e, t, n, r) {
    ...
    for (; ; ) {
      ...
      try {
        ...
        if (__PRIVATE_logDebug(Q, "Transaction failed with error:", t2.message, "Retrying:", n2), this.close(), !n2) return Promise.reject(t2);
      }
    }
  }
  close() {
...
var __PRIVATE_IndexedDbTransactionError = class extends FirestoreError {
  constructor(e, t) {
    super(x.UNAVAILABLE, `IndexedDB transaction '${e}' failed: ${t}`), this.name = "IndexedDbTransactionError";
  }
};
...
var __PRIVATE_SimpleDbStore = class {
  ...
  put(e, t) {
    let n;
    return void 0 !== t ? (__PRIVATE_logDebug(Q, "PUT", this.store.name, e, t), n = this.store.put(t, e)) : (__PRIVATE_logDebug(Q, "PUT", this.store.name, "<auto-key>", e), n = this.store.put(e)), __PRIVATE_wrapRequest(n);
  }
  /**
  ...
  get(e) {
    return __PRIVATE_wrapRequest(this.store.get(e)).next((t) => (
      ...
      (void 0 === t && (t = null), __PRIVATE_logDebug(Q, "GET", this.store.name, e, t), t)
    ));
  }
  delete(e) {
...
function __PRIVATE_checkForAndReportiOSError(e) {
  ...
  if (t >= 12.2 && t < 13) {
    ...
    if (e.message.indexOf(t2) >= 0) {
      ...
      return $ || ($ = true, // Throw a global exception outside of this promise chain, for the user to
      // potentially catch.
      setTimeout(() => {
        throw e2;
      }, 0)), e2;
...
var __PRIVATE_IndexBackfillerScheduler = class {
  ...
  ie(e) {
    __PRIVATE_logDebug(U, `Scheduled in ${e}ms`), this.task = this.asyncQueue.enqueueAfterDelay("index_backfill", e, async () => {
      this.task = null;
      try {
        const e2 = await this.re.se();
        __PRIVATE_logDebug(U, `Documents written: ${e2}`);
      } catch (e2) {
        __PRIVATE_isIndexedDbTransactionError(e2) ? __PRIVATE_logDebug(U, "Ignoring IndexedDB error during index backfill: ", e2) : await __PRIVATE_ignoreIfPrimaryLeaseLoss(e2);
      }
      await this.ie(6e4);
    });
  }
...
var __PRIVATE_IndexBackfiller = class {
  ...
  oe(e, t) {
    ...
    return PersistencePromise.doWhile(() => true === i && r > 0, () => this.localStore.indexManager.getNextCollectionGroupToUpdate(e).next((t2) => {
      if (null !== t2 && !n.has(t2)) return __PRIVATE_logDebug(U, `Processing collection: ${t2}`), this._e(e, t2, r).next((e2) => {
        r -= e2, n.add(t2);
      });
      i = false;
    })).next(() => t - r);
  ...
  _e(e, t, n) {
    return this.localStore.indexManager.getMinOffsetFromCollectionGroup(e, t).next((r) => this.localStore.localDocuments.getNextDocuments(e, t, r, n).next((n2) => {
      const i = n2.changes;
      return this.localStore.indexManager.updateIndexEntries(e, i).next(() => this.ae(r, n2)).next((n3) => (__PRIVATE_logDebug(U, `Updating offset: ${n3}`), this.localStore.indexManager.updateCollectionGroup(e, t, n3))).next(() => i.size);
    }));
  }
...
function __PRIVATE_decodeResourcePath(e) {
  ...
  if (__PRIVATE_hardAssert(t >= 2, 64408, {
    path: e
  }), 2 === t) return __PRIVATE_hardAssert(e.charAt(0) === W && "" === e.charAt(1), 56145, {
    path: e
  }), ResourcePath.emptyPath();
  const __PRIVATE_lastReasonableEscapeIndex = t - 2, n = [];
  let r = "";
  for (let i = 0; i < t; ) {
    const t2 = e.indexOf(W, i);
...
var G = "remoteDocuments";
var z = "owner";
var j = "owner";
var H = "mutationQueues";
var J = "userId";
var Y = "mutations";
var Z = "batchId";
var X = "userMutationsIndex";
var ee = ["userId", "batchId"];
...
var te = {};
var ne = "documentMutations";
var re = "remoteDocumentsV14";
var ie = ["prefixPath", "collectionGroup", "readTime", "documentId"];
var se = "documentKeyIndex";
var oe = ["prefixPath", "collectionGroup", "documentId"];
var _e = "collectionGroupIndex";
var ae = ["collectionGroup", "readTime", "prefixPath", "documentId"];
var ue = "remoteDocumentGlobal";
var ce = "remoteDocumentGlobalKey";
var le = "targets";
var he = "queryTargetsIndex";
var Pe = ["canonicalId", "targetId"];
var Te = "targetDocuments";
var Ie = ["targetId", "path"];
var Ee = "documentTargetsIndex";
var de = ["path", "targetId"];
var Ae = "targetGlobalKey";
var Re = "targetGlobal";
var Ve = "collectionParents";
var me = ["collectionId", "parent"];
var fe = "clientMetadata";
var ge = "clientId";
var pe = "bundles";
var ye = "bundleId";
var we = "namedQueries";
var be = "name";
var Se = "indexConfiguration";
var De = "indexId";
var ve = "collectionGroupIndex";
var Ce = "collectionGroup";
var Fe = "indexState";
var Me = ["indexId", "uid"];
var xe = "sequenceNumberIndex";
var Oe = ["uid", "sequenceNumber"];
var Ne = "indexEntries";
var Be = ["indexId", "uid", "arrayValue", "directionalValue", "orderedDocumentKey", "documentKey"];
var Le = "documentKeyIndex";
var ke = ["indexId", "uid", "orderedDocumentKey"];
var qe = "documentOverlays";
var Qe = ["userId", "collectionPath", "documentId"];
var $e = "collectionPathOverlayIndex";
var Ue = ["userId", "collectionPath", "largestBatchId"];
var Ke = "collectionGroupOverlayIndex";
var We = ["userId", "collectionGroup", "largestBatchId"];
var Ge = "globals";
var ze = "name";
var je = [...[...[...[...[H, Y, ne, G, le, z, Re, Te], fe], ue], Ve], pe, we];
var He = [...je, qe];
var Je = [H, Y, ne, re, le, z, Re, Te, fe, ue, Ve, pe, we, qe];
var Ye = Je;
var Ze = [...Ye, Se, Fe, Ne];
var Xe = Ze;
var et = [...Ze, Ge];
...
var nt = "server_timestamp";
var rt = "__type__";
var it = "__previous_value__";
var st = "__local_write_time__";
function __PRIVATE_isServerTimestamp(e) {
  var t, n;
  return (null === (n = ((null === (t = null == e ? void 0 : e.mapValue) || void 0 === t ? void 0 : t.fields) || {})[rt]) || void 0 === n ? void 0 : n.stringValue) === nt;
}
function __PRIVATE_getPreviousValue(e) {
  const t = e.mapValue.fields[it];
  return __PRIVATE_isServerTimestamp(t) ? __PRIVATE_getPreviousValue(t) : t;
}
function __PRIVATE_getLocalWriteTime(e) {
  const t = __PRIVATE_normalizeTimestamp(e.mapValue.fields[st].timestampValue);
...
var DatabaseId = class _DatabaseId {
  constructor(e, t) {
    this.projectId = e, this.database = t || ot;
  }
  static empty() {
    return new _DatabaseId("", "");
  }
  get isDefaultDatabase() {
    return this.database === ot;
  }
  isEqual(e) {
  ...
  switch (n) {
    ...
    case 10:
      return function __PRIVATE_compareVectors(e2, t2) {
        var n2, r2, i, s;
        const o = e2.fields || {}, _ = t2.fields || {}, a = null === (n2 = o[lt]) || void 0 === n2 ? void 0 : n2.arrayValue, u = null === (r2 = _[lt]) || void 0 === r2 ? void 0 : r2.arrayValue, c = __PRIVATE_primitiveComparator((null === (i = null == a ? void 0 : a.values) || void 0 === i ? void 0 : i.length) || 0, (null === (s = null == u ? void 0 : u.values) || void 0 === s ? void 0 : s.length) || 0);
        if (0 !== c) return c;
        return __PRIVATE_compareArrays(a, u);
      }(e.mapValue, t.mapValue);
    case 11:
      return function __PRIVATE_compareMaps(e2, t2) {
        if (e2 === ut.mapValue && t2 === ut.mapValue) return 0;
        if (e2 === ut.mapValue) return 1;
        if (t2 === ut.mapValue) return -1;
...
function __PRIVATE_isVectorValue(e) {
  var t, n;
  return (null === (n = ((null === (t = null == e ? void 0 : e.mapValue) || void 0 === t ? void 0 : t.fields) || {})[_t]) || void 0 === n ? void 0 : n.stringValue) === ct;
}
...
function __PRIVATE_isMaxValue(e) {
  return (((e.mapValue || {}).fields || {}).__type__ || {}).stringValue === at;
}
var Pt = {
  mapValue: {
    fields: {
...
function __PRIVATE_valuesGetLowerBound(e) {
  return "nullValue" in e ? ht : "booleanValue" in e ? {
    booleanValue: false
    ...
    arrayValue: {}
  } : "mapValue" in e ? __PRIVATE_isVectorValue(e) ? Pt : {
    mapValue: {}
  } : fail(35942, {
    value: e
  });
...
function __PRIVATE_valuesGetUpperBound(e) {
  return "nullValue" in e ? {
    ...
    arrayValue: {}
  } : "arrayValue" in e ? Pt : "mapValue" in e ? __PRIVATE_isVectorValue(e) ? {
    mapValue: {}
  } : ut : fail(61959, {
    value: e
  });
...
var dt = new SortedSet(DocumentKey.comparator);
function __PRIVATE_documentKeySet(...e) {
  let t = dt;
  for (const n of e) t = t.add(n);
  return t;
}
var At = new SortedSet(__PRIVATE_primitiveComparator);
...
function __PRIVATE_applyTransformOperationToLocalView(e, t, n) {
  return e instanceof __PRIVATE_ServerTimestampTransform ? function serverTimestamp$1(e2, t2) {
    ...
    return t2 && __PRIVATE_isServerTimestamp(t2) && (t2 = __PRIVATE_getPreviousValue(t2)), t2 && (n2.fields[it] = t2), {
      mapValue: n2
    };
...
function __PRIVATE_mapCodeFromRpcCode(e) {
  if (void 0 === e)
    return __PRIVATE_logError("GRPC error has no .code"), x.UNKNOWN;
  switch (e) {
    case Rt.OK:
      return x.OK;
    case Rt.CANCELLED:
...
(Vt = Rt || (Rt = {}))[Vt.OK = 0] = "OK", Vt[Vt.CANCELLED = 1] = "CANCELLED", Vt[Vt.UNKNOWN = 2] = "UNKNOWN", Vt[Vt.INVALID_ARGUMENT = 3] = "INVALID_ARGUMENT", Vt[Vt.DEADLINE_EXCEEDED = 4] = "DEADLINE_EXCEEDED", Vt[Vt.NOT_FOUND = 5] = "NOT_FOUND", Vt[Vt.ALREADY_EXISTS = 6] = "ALREADY_EXISTS", Vt[Vt.PERMISSION_DENIED = 7] = "PERMISSION_DENIED", Vt[Vt.UNAUTHENTICATED = 16] = "UNAUTHENTICATED", Vt[Vt.RESOURCE_EXHAUSTED = 8] = "RESOURCE_EXHAUSTED", Vt[Vt.FAILED_PRECONDITION = 9] = "FAILED_PRECONDITION", Vt[Vt.ABORTED = 10] = "ABORTED", Vt[Vt.OUT_OF_RANGE = 11] = "OUT_OF_RANGE", Vt[Vt.UNIMPLEMENTED = 12] = "UNIMPLEMENTED", Vt[Vt.INTERNAL = 13] = "INTERNAL", Vt[Vt.UNAVAILABLE = 14] = "UNAVAILABLE", Vt[Vt.DATA_LOSS = 15] = "DATA_LOSS";
var mt = null;
var ft = new Integer([4294967295, 4294967295], 0);
...
var BloomFilter = class _BloomFilter {
  ...
  we(e, t, n) {
    ...
    return 1 === r.compare(ft) && (r = new Integer([r.getBits(0), r.getBits(1)], 0)), r.modulo(this.ye).toNumber();
  }
    ...
    if (r) {
      ...
      if (__PRIVATE_targetIsDocumentTarget(i)) if (0 === n) {
        ...
        if (r2 !== n) {
          ...
          null == mt || mt.ht(function __PRIVATE_createExistenceFilterMismatchInfoForTestingHooks(e2, t2, n3, r3, i3) {
            var s, o, _, a, u, c;
            const l = {
              localCacheCount: e2,
...
function fromName(e, t) {
  ...
  if (n.get(1) !== e.databaseId.projectId) throw new FirestoreError(x.INVALID_ARGUMENT, "Tried to deserialize key from different project: " + n.get(1) + " vs " + e.databaseId.projectId);
  if (n.get(3) !== e.databaseId.database) throw new FirestoreError(x.INVALID_ARGUMENT, "Tried to deserialize key from different database: " + n.get(3) + " vs " + e.databaseId.database);
...
function __PRIVATE_fromWatchChange(e, t) {
  ...
  if ("targetChange" in t) {
    ...
    const r = function __PRIVATE_fromWatchTargetChangeState(e2) {
      ...
      const t2 = void 0 === e2.code ? x.UNKNOWN : __PRIVATE_mapCodeFromRpcCode(e2.code);
...
var __PRIVATE_IndexedDbDocumentOverlayCache = class ___PRIVATE_IndexedDbDocumentOverlayCache {
  ...
  removeOverlaysForBatchId(e, t, n) {
    ...
    return r.forEach((t2) => {
      ...
      i.push(__PRIVATE_documentOverlayStore(e).X($e, r2));
  ...
  getOverlaysForCollection(e, t, n) {
    ...
    return __PRIVATE_documentOverlayStore(e).J($e, s).next((e2) => {
      for (const t2 of e2) {
...
var __PRIVATE_FirestoreIndexValueWriter = class {
  ...
  qt(e, t) {
    ...
    const i = e.fields || {};
    this.Mt(t, 53);
    const s = lt, o = (null === (r = null === (n = i[s].arrayValue) || void 0 === n ? void 0 : n.values) || void 0 === r ? void 0 : r.length) || 0;
...
var __PRIVATE_OrderedCodeWriter = class {
  ...
  zt(e) {
    const t = 255 & e;
    0 === t ? (this._n(0), this._n(255)) : t === wt ? (this._n(wt), this._n(0)) : this._n(t);
  }
  /** Writes a single byte descending to the buffer.  */
  Jt(e) {
    const t = 255 & e;
    0 === t ? (this.un(0), this.un(255)) : t === wt ? (this.un(wt), this.un(0)) : this.un(e);
  }
  jt() {
    this._n(0), this._n(1);
  }
...
var __PRIVATE_IndexedDbIndexManager = class {
    ...
    return PersistencePromise.forEach(this.Sn(t), (t2) => this.Dn(e, t2).next((e2) => {
      ...
      if (r) {
        ...
        return PersistencePromise.forEach(i, (i2, s) => {
          __PRIVATE_logDebug(bt, `Using index ${function __PRIVATE_fieldIndexToString(e3) {
  ...
  Fn(e, t, n, r, i, s, o) {
    ...
    for (let c = 0; c < _; ++c) {
      const _2 = t ? this.Mn(t[c / a]) : St, l = this.xn(e, _2, n[c % a], r), h = this.On(e, _2, i[c % a], s), P = o.map((t2) => this.xn(
        e,
        _2,
        t2,
        /* inclusive= */
  ...
  getFieldIndexes(e, t) {
    ...
    return (t ? n.J(ve, IDBKeyRange.bound(t, t)) : n.J()).next((e2) => {
      const t2 = [];
  ...
  updateCollectionGroup(e, t, n) {
    ...
    return this.Qn(e).next((e2) => r.J(ve, IDBKeyRange.bound(t, t)).next((t2) => PersistencePromise.forEach(t2, (t3) => i.put(__PRIVATE_toDbIndexState(t3.indexId, this.uid, e2, n)))));
  }
  updateIndexEntries(e, t) {
  ...
  createRange(e, t, n) {
    ...
    for (let e2 = 0; e2 < r.length; e2 += 2) {
      ...
      const t2 = [r[e2].indexId, this.uid, r[e2].arrayValue, r[e2].directionalValue, St, []], n2 = [r[e2 + 1].indexId, this.uid, r[e2 + 1].arrayValue, r[e2 + 1].directionalValue, St, []];
...
function removeMutationBatch(e, t, n) {
  const r = e.store(Y), i = e.store(ne), s = [], o = IDBKeyRange.only(n.batchId);
  let _ = 0;
  const a = r.te({
    range: o
...
LruParams.DEFAULT_COLLECTION_PERCENTILE = 10, LruParams.DEFAULT_MAX_SEQUENCE_NUMBERS_TO_COLLECT = 1e3, LruParams.DEFAULT = new LruParams(vt, LruParams.DEFAULT_COLLECTION_PERCENTILE, LruParams.DEFAULT_MAX_SEQUENCE_NUMBERS_TO_COLLECT), LruParams.DISABLED = new LruParams(-1, 0, 0);
var __PRIVATE_IndexedDbMutationQueue = class ___PRIVATE_IndexedDbMutationQueue {
  ...
  addMutationBatch(e, t, n, r) {
    ...
    return s.add({}).next((o) => {
      ...
      for (const e2 of r) {
        ...
        c = c.add(e2.key.path.popLast()), u.push(s.put(a)), u.push(i.put(t2, te));
      }
      return c.forEach((t2) => {
  ...
  getAllMutationBatches(e) {
    ...
    return __PRIVATE_mutationsStore(e).J(X, t).next((e2) => e2.map((e3) => __PRIVATE_fromDbMutationBatch(this.serializer, e3)));
  }
...
var __PRIVATE_IndexedDbTargetCache = class {
  ...
  sr(e) {
    return __PRIVATE_globalTargetStore(e).get(Ae).next((e2) => (__PRIVATE_hardAssert(null !== e2, 2888), e2));
  }
  _r(e, t) {
...
var __PRIVATE_LruScheduler = class {
  ...
  Ir(e) {
    __PRIVATE_logDebug(Ct, `Garbage collection scheduled in ${e}ms`), this.Tr = this.asyncQueue.enqueueAfterDelay("lru_garbage_collection", e, async () => {
      this.Tr = null;
      try {
...
var __PRIVATE_IndexedDbRemoteDocumentCacheImpl = class {
  ...
  getMetadata(e) {
    return __PRIVATE_documentGlobalStore(e).get(ce).next((e2) => (__PRIVATE_hardAssert(!!e2, 20021), e2));
  }
  pr(e, t) {
...
var __PRIVATE_MemoryMutationQueue = class {
  ...
  getHighestUnacknowledgedBatchId() {
    return PersistencePromise.resolve(0 === this.mutationQueue.length ? K : this.Jn - 1);
  }
  getAllMutationBatches(e) {
...
var __PRIVATE_SchemaConverter = class {
  ...
  q(e, t, n, r) {
    ...
    n < 1 && r >= 1 && (!function __PRIVATE_createPrimaryClientStore(e2) {
      ...
      const t2 = e2.createObjectStore(Y, {
        keyPath: Z,
        autoIncrement: true
      });
      t2.createIndex(X, ee, {
        unique: true
      }), e2.createObjectStore(ne);
    ...
    (0 !== n && (!function __PRIVATE_dropQueryCache(e2) {
      e2.deleteObjectStore(Te), e2.deleteObjectStore(le), e2.deleteObjectStore(Re);
    ...
    (s = s.next(() => function __PRIVATE_upgradeMutationBatchSchemaAndMigrateData(e2, t2) {
      ...
      return n2.J().next((n3) => {
        ...
        const r2 = t2.store(Y), i2 = n3.map((e3) => r2.put(e3));
      ...
      const t2 = e2.createObjectStore(re, {
        keyPath: ie
      });
      t2.createIndex(se, oe), t2.createIndex(_e, ae);
    }(e)).next(() => this.yi(e, i)).next(() => e.deleteObjectStore(G))), n < 14 && r >= 14 && (s = s.next(() => this.wi(e, i))), n < 15 && r >= 15 && (s = s.next(() => function __PRIVATE_createFieldIndex(e2) {
      const t2 = e2.createObjectStore(Se, {
        keyPath: De,
        autoIncrement: true
      });
      t2.createIndex(ve, Ce, {
        unique: false
      });
      const n2 = e2.createObjectStore(Fe, {
        keyPath: Me
      });
      n2.createIndex(xe, Oe, {
        unique: false
      });
      const r2 = e2.createObjectStore(Ne, {
        keyPath: Be
      });
      r2.createIndex(Le, ke, {
        unique: false
      });
  ...
  Vi(e) {
    const t = e.store(H), n = e.store(Y);
    return t.J().next((t2) => PersistencePromise.forEach(t2, (t3) => {
      const r = IDBKeyRange.bound([t3.userId, K], [t3.userId, t3.lastAcknowledgedBatchId]);
      return n.J(X, r).next((n2) => PersistencePromise.forEach(n2, (n3) => {
   ...
   */
    ...
    return e.store(Re).get(Ae).next((e2) => {
      const r = [];
      return n.te((n2, i) => {
  ...
  gi(e, t) {
    ...
    const n = t.store(Ve), r = new __PRIVATE_MemoryCollectionParentIndex(), addEntry = (e2) => {
      if (r.add(e2)) {
  ...
  yi(e, t) {
    const n = t.store(G), r = [];
    return n.te((e2, n2) => {
      const i = t.store(re), s = function __PRIVATE_extractKey(e3) {
  ...
  wi(e, t) {
    const n = t.store(Y), r = __PRIVATE_newIndexedDbRemoteDocumentCache(this.serializer), i = new __PRIVATE_MemoryPersistence(__PRIVATE_MemoryEagerDelegate.Ei, this.serializer.wt);
    return n.J().next((e2) => {
      const n2 = /* @__PURE__ */ new Map();
...
var Mt = "IndexedDbPersistence";
var xt = 18e5;
var Ot = 5e3;
var Nt = "Failed to obtain exclusive access to the persistence layer. To allow shared access, multi-tab synchronization has to be enabled in all tabs. If you are using `experimentalForceOwningTab:true`, make sure that only one tab has persistence enabled at any given time.";
var Bt = "main";
var __PRIVATE_IndexedDbPersistence = class ___PRIVATE_IndexedDbPersistence {
  constructor(e, t, n, r, i, s, o, _, a, u, c = 17) {
    ...
    this.Ni = (e2) => Promise.resolve(), !___PRIVATE_IndexedDbPersistence.C()) throw new FirestoreError(x.UNIMPLEMENTED, "This platform is either missing IndexedDB or is known to have an incomplete implementation. Offline persistence has been disabled.");
    this.referenceDelegate = new __PRIVATE_IndexedDbLruDelegateImpl(this, r), this.Bi = t + Bt, this.serializer = new __PRIVATE_LocalSerializer(_), this.Li = new __PRIVATE_SimpleDb(this.Bi, this.Ci, new __PRIVATE_SchemaConverter(this.serializer)), this._i = new __PRIVATE_IndexedDbGlobalsCache(), this.ai = new __PRIVATE_IndexedDbTargetCache(this.referenceDelegate, this.serializer), this.remoteDocumentCache = __PRIVATE_newIndexedDbRemoteDocumentCache(this.serializer), this.ci = new __PRIVATE_IndexedDbBundleCache(), this.window && this.window.localStorage ? this.ki = this.window.localStorage : (this.ki = null, false === u && __PRIVATE_logError(Mt, "LocalStorage is unavailable. As a result, persistence may not work reliably. In particular enablePersistence() could fail immediately after refreshing the page."));
  }
  /**
  ...
  start() {
    return this.qi().then(() => {
      if (!this.isPrimary && !this.allowTabSynchronization)
        throw new FirestoreError(x.FAILED_PRECONDITION, Nt);
  ...
  qi() {
    return this.runTransaction("updateClientMetadataAndTryBecomePrimary", "readwrite", (e) => __PRIVATE_clientMetadataStore(e).put({
      ...
      if (__PRIVATE_isIndexedDbTransactionError(e))
        return __PRIVATE_logDebug(Mt, "Failed to extend owner lease: ", e), this.isPrimary;
      if (!this.allowTabSynchronization) throw e;
      return __PRIVATE_logDebug(Mt, "Releasing owner lease after error during lease refresh", e), /* isPrimary= */
      false;
    }).then((e) => {
  ...
  Wi(e) {
    return __PRIVATE_primaryClientStore(e).get(j).next((e2) => PersistencePromise.resolve(this.Hi(e2)));
  }
  Ji(e) {
   ...
   */
  async Yi() {
    if (this.isPrimary && !this.Zi(this.Oi, xt)) {
      this.Oi = Date.now();
      const e = await this.runTransaction("maybeGarbageCollectMultiClientState", "readwrite-primary", (e2) => {
        const t = __PRIVATE_getStore(e2, fe);
        return t.J().next((e3) => {
          const n = this.Xi(e3, xt), r = e3.filter((e4) => -1 === n.indexOf(e4));
  ...
  Gi(e) {
    ...
    return __PRIVATE_primaryClientStore(e).get(j).next((t) => {
      if (null !== t && this.Zi(t.leaseTimestampMs, Ot) && !this.ts(t.ownerId)) {
      ...
      return !(!this.networkEnabled || !this.inForeground) || __PRIVATE_clientMetadataStore(e).J().next((e2) => void 0 === this.Xi(e2, Ot).find((e3) => {
    ...
    }).next((e2) => (this.isPrimary !== e2 && __PRIVATE_logDebug(Mt, `Client ${e2 ? "is" : "is not"} eligible for a primary lease.`), e2));
  }
  async shutdown() {
    ...
    await this.Li.runTransaction("shutdown", "readwrite", [z, fe], (e) => {
   ...
   */
  us() {
    return this.runTransaction("getActiveClients", "readonly", (e) => __PRIVATE_clientMetadataStore(e).J().next((e2) => this.Xi(e2, xt).map((e3) => e3.clientId)));
  }
  get started() {
    return this.oi;
  }
  ...
  runTransaction(e, t, n) {
    __PRIVATE_logDebug(Mt, "Starting transaction:", e);
    const r = "readonly" === t ? "readonly" : "readwrite", i = (
      /** Returns the object stores for the provided schema. */
      function __PRIVATE_getObjectStores(e2) {
        return 17 === e2 ? et : 16 === e2 ? Xe : 15 === e2 ? Ze : 14 === e2 ? Ye : 13 === e2 ? Je : 12 === e2 ? He : 11 === e2 ? je : void fail(60245);
      }(this.Ci)
    );
    let s;
    return this.Li.runTransaction(e, r, i, (r2) => (s = new __PRIVATE_IndexedDbTransaction(r2, this.si ? this.si.next() : __PRIVATE_ListenSequence.le), "readwrite-primary" === t ? this.Wi(s).next((e2) => !!e2 || this.Gi(s)).next((t2) => {
      if (!t2) throw __PRIVATE_logError(`Failed to obtain primary lease for action '${e}'.`), this.isPrimary = false, this.bi.enqueueRetryable(() => this.Ni(false)), new FirestoreError(x.FAILED_PRECONDITION, q);
      return n(s);
  ...
  cs(e) {
    return __PRIVATE_primaryClientStore(e).get(j).next((e2) => {
      if (null !== e2 && this.Zi(e2.leaseTimestampMs, Ot) && !this.ts(e2.ownerId) && !this.Hi(e2) && !(this.Di || this.allowTabSynchronization && e2.allowTabSynchronization)) throw new FirestoreError(x.FAILED_PRECONDITION, Nt);
    });
  }
  /**
  ...
  zi(e) {
    ...
    return t.get(j).next((e2) => this.Hi(e2) ? (__PRIVATE_logDebug(Mt, "Releasing primary lease."), t.delete(j)) : PersistencePromise.resolve());
  }
  ...
  ts(e) {
    ...
    try {
      ...
      return __PRIVATE_logDebug(Mt, `Client '${e}' ${n ? "is" : "is not"} zombied in LocalStorage`), n;
    } catch (e2) {
      return __PRIVATE_logError(Mt, "Failed to get zombied client id.", e2), false;
    }
  }
  /**
...
var __PRIVATE_QueryEngine = class {
  ...
  Vs(e, t, n, r) {
    return __PRIVATE_queryMatchesAllDocuments(t) || r.isEqual(SnapshotVersion.min()) ? PersistencePromise.resolve(null) : this.As.getDocuments(e, n).next((i) => {
      const s = this.ps(t, i);
      return this.ys(t, s, n, r) ? PersistencePromise.resolve(null) : (__PRIVATE_getLogLevel() <= LogLevel.DEBUG && __PRIVATE_logDebug("QueryEngine", "Re-using previous result from %s to execute query: %s", r.toString(), __PRIVATE_stringifyQuery(t)), this.ws(e, s, t, __PRIVATE_newIndexOffsetSuccessorFromReadTime(r, k)).next((e2) => e2));
    });
  }
...
function __PRIVATE_populateDocumentChangeBuffer(e, t, n) {
  ...
  return n.forEach((e2) => r = r.add(e2)), t.getEntries(e, r).next((e2) => {
    ...
    return n.forEach((n2, s) => {
      ...
      ) : !o.isValidDocument() || s.version.compareTo(o.version) > 0 || 0 === s.version.compareTo(o.version) && o.hasPendingWrites ? (t.addEntry(s), r2 = r2.insert(n2, s)) : __PRIVATE_logDebug(Lt, "Ignoring outdated watch update for ", n2, ". Current version:", o.version, " Watch version:", s.version);
    }), {
      xs: r2,
      Os: i
    };
...
function __PRIVATE_localStoreGetNextMutationBatch(e, t) {
  const n = __PRIVATE_debugCast(e);
  return n.persistence.runTransaction("Get next mutation batch", "readonly", (e2) => (void 0 === t && (t = K), n.mutationQueue.getNextMutationBatchAfterBatchId(e2, t)));
}
...
async function __PRIVATE_localStoreReleaseTarget(e, t, n) {
  ...
  try {
    ...
    __PRIVATE_logDebug(Lt, `Failed to update sequence numbers for target ${t}: ${e2}`);
  }
...
function __PRIVATE_localStoreGetNewDocumentChanges(e, t) {
  ...
  return n.persistence.runTransaction("Get new document changes", "readonly", (e2) => n.Cs.getAllFromCollectionGroup(
    e2,
    t,
    __PRIVATE_newIndexOffsetSuccessorFromReadTime(r, k),
    /* limit= */
...
var __PRIVATE_WebStorageSharedClientState = class {
  constructor(e, t, n, r, i) {
    ...
    }(this.persistenceKey), this.zs = this.zs.insert(this.Ks, new __PRIVATE_LocalClientState()), this.Ys = new RegExp(`^${qt}_${s}_([^_]*)$`), this.Zs = new RegExp(`^${Qt}_${s}_(\\d+)(?:_(.*))?$`), this.Xs = new RegExp(`^${$t}_${s}_(\\d+)$`), this.eo = /** Assembles the key for the online state of the primary tab. */
  ...
  Gs(e) {
    ...
    if (t.storageArea === this.storage) {
      if (__PRIVATE_logDebug(Ut, "EVENT", t.key, t.newValue), t.key === this.Hs) return void __PRIVATE_logError("Received WebStorage notification for local change. Another client might have garbage-collected our state");
...
var __PRIVATE_RestConnection = class {
  ...
  constructor(e) {
    ...
    this.Lo = t + "://" + e.host, this.ko = `projects/${n}/databases/${r}`, this.qo = this.databaseId.database === ot ? `project_id=${n}` : `project_id=${n}&database_id=${r}`;
  }
  Qo(e, t, n, r, i) {
    ...
    return this.Ko(e, o, _, n, u).then((t2) => (__PRIVATE_logDebug(Gt, `Received RPC '${e}' ${s}: `, t2), t2), (t2) => {
      throw __PRIVATE_logWarn(Gt, `RPC '${e}' ${s} failed with error: `, t2, "url: ", o, "request:", n), t2;
    });
  }
  Wo(e, t, n, r, i, s) {
...
var __PRIVATE_WebChannelConnection = class extends __PRIVATE_RestConnection {
        ...
        try {
          switch (_.getLastErrorCode()) {
            case ErrorCode.NO_ERROR:
              ...
              __PRIVATE_logDebug(jt, `XHR for RPC '${e}' ${s} received:`, JSON.stringify(t2)), i2(t2);
              break;
            case ErrorCode.TIMEOUT:
              __PRIVATE_logDebug(jt, `RPC '${e}' ${s} timed out`), o(new FirestoreError(x.DEADLINE_EXCEEDED, "Request time out"));
              break;
            case ErrorCode.HTTP_ERROR:
              const n2 = _.getStatus();
              if (__PRIVATE_logDebug(jt, `RPC '${e}' ${s} failed with status:`, n2, "response text:", _.getResponseText()), n2 > 0) {
                ...
                if (t3 && t3.status && t3.message) {
                  const e3 = function __PRIVATE_mapCodeFromHttpResponseErrorStatus(e4) {
                    ...
                    return Object.values(x).indexOf(t4) >= 0 ? t4 : x.UNKNOWN;
                  }(t3.status);
                  o(new FirestoreError(e3, t3.message));
                } else o(new FirestoreError(x.UNKNOWN, "Server responded with status " + _.getStatus()));
              } else
                o(new FirestoreError(x.UNAVAILABLE, "Connection failed."));
              break;
            default:
              fail(9055, {
                s_: e,
  ...
  a_(e, t, n) {
    ...
    const u = i.join("");
    __PRIVATE_logDebug(jt, `Creating RPC '${e}' stream ${r}: ${u}`, _);
    const c = s.createWebChannel(u, _);
    let l = false, h = false;
    const P = new __PRIVATE_StreamBridge({
      Go: (t2) => {
        h ? __PRIVATE_logDebug(jt, `Not sending because RPC '${e}' stream ${r} is closed:`, t2) : (l || (__PRIVATE_logDebug(jt, `Opening RPC '${e}' stream ${r} transport.`), c.open(), l = true), __PRIVATE_logDebug(jt, `RPC '${e}' stream ${r} sending:`, t2), c.send(t2));
      },
      zo: () => c.close()
    ...
    return __PRIVATE_unguardedEventListen(c, WebChannel.EventType.OPEN, () => {
      h || (__PRIVATE_logDebug(jt, `RPC '${e}' stream ${r} transport opened.`), P.t_());
    }), __PRIVATE_unguardedEventListen(c, WebChannel.EventType.CLOSE, () => {
      h || (h = true, __PRIVATE_logDebug(jt, `RPC '${e}' stream ${r} transport closed`), P.r_());
    }), __PRIVATE_unguardedEventListen(c, WebChannel.EventType.ERROR, (t2) => {
      h || (h = true, __PRIVATE_logWarn(jt, `RPC '${e}' stream ${r} transport errored. Name:`, t2.name, "Message:", t2.message), P.r_(new FirestoreError(x.UNAVAILABLE, "The operation could not be completed")));
      ...
      if (!h) {
        ...
        if (o2) {
          __PRIVATE_logDebug(jt, `RPC '${e}' stream ${r} received error:`, o2);
          const t3 = o2.status;
          let n3 = (
            /**
          ...
          void 0 === n3 && (n3 = x.INTERNAL, i3 = "Unknown error status: " + t3 + " with message " + o2.message), // Mark closed so no further events are propagated
          h = true, P.r_(new FirestoreError(n3, i3)), c.close();
        } else __PRIVATE_logDebug(jt, `RPC '${e}' stream ${r} received:`, i2), P.i_(i2);
      }
    }), __PRIVATE_unguardedEventListen(o, Event.STAT_EVENT, (t2) => {
      t2.stat === Stat.PROXY ? __PRIVATE_logDebug(jt, `RPC '${e}' stream ${r} detected buffering proxy`) : t2.stat === Stat.NOPROXY && __PRIVATE_logDebug(jt, `RPC '${e}' stream ${r} detected no buffering proxy`);
    }), setTimeout(() => {
      P.n_();
    }, 0), P;
  }
...
var __PRIVATE_PersistentStream = class {
  ...
  async close(e, t) {
    ...
    ) : t && t.code === x.UNAUTHENTICATED && 3 !== this.state && // "unauthenticated" error means the token was rejected. This should rarely
  ...
  auth() {
    ...
    Promise.all([this.authCredentialsProvider.getToken(), this.appCheckCredentialsProvider.getToken()]).then(([e2, n]) => {
      ...
      e(() => {
        const e2 = new FirestoreError(x.UNKNOWN, "Fetching auth token failed: " + t2.message);
        return this.L_(e2);
      });
    });
  }
  ...
  // Visible for tests
  L_(e) {
    return __PRIVATE_logDebug(Ht, `close with error: ${e}`), this.stream = null, this.close(4, e);
  }
  /**
  ...
  N_(e) {
    return (t) => {
      this.bi.enqueueAndForget(() => this.m_ === e ? t() : (__PRIVATE_logDebug(Ht, "stream callback skipped by getCloseGuardedDispatcher."), Promise.resolve()));
    };
  }
};
...
var __PRIVATE_DatastoreImpl = class extends Datastore {
  ...
  Y_() {
    if (this.J_) throw new FirestoreError(x.FAILED_PRECONDITION, "The client has already been terminated.");
  }
  ...
  Qo(e, t, n, r) {
    return this.Y_(), Promise.all([this.authCredentials.getToken(), this.appCheckCredentials.getToken()]).then(([i, s]) => this.connection.Qo(e, __PRIVATE_toResourcePath(t, n), r, i, s)).catch((e2) => {
      throw "FirebaseError" === e2.name ? (e2.code === x.UNAUTHENTICATED && (this.authCredentials.invalidateToken(), this.appCheckCredentials.invalidateToken()), e2) : new FirestoreError(x.UNKNOWN, e2.toString());
    });
  }
...
async function __PRIVATE_fillWritePipeline(e) {
  ...
  let r = t.oa.length > 0 ? t.oa[t.oa.length - 1].batchId : K;
...
async function __PRIVATE_onWriteStreamClose(e, t) {
  ...
  await async function __PRIVATE_handleWriteError(e2, t2) {
    if (function __PRIVATE_isPermanentWriteError(e3) {
      return __PRIVATE_isPermanentError(e3) && e3 !== x.ABORTED;
    }(t2.code)) {
      const n = e2.oa.shift();
...
var DelayedOperation = class _DelayedOperation {
   ...
   */
  cancel(e) {
    null !== this.timerHandle && (this.clearTimeout(), this.deferred.reject(new FirestoreError(x.CANCELLED, "Operation cancelled" + (e ? ": " + e : ""))));
  }
  handleDelayElapsed() {
...
function __PRIVATE_wrapInUserErrorIfRecoverable(e, t) {
  if (__PRIVATE_logError("AsyncQueue", `${t}: ${e}`), __PRIVATE_isIndexedDbTransactionError(e)) return new FirestoreError(x.UNAVAILABLE, `${t}: ${e}`);
  throw e;
}
...
var __PRIVATE_EventManagerImpl = class {
  ...
  terminate() {
    ...
    }(this, new FirestoreError(x.ABORTED, "Firestore shutting down"));
  }
};
...
var Yt;
var Zt;
(Zt = Yt || (Yt = {})).ya = "default", /** Listen to changes in cache only */
Zt.Cache = "cache";
...
function __PRIVATE_updateTrackedLimbos(e, t, n) {
  for (const r of n) if (r instanceof __PRIVATE_AddedLimboDocument) e.au.addReference(r.key, t), __PRIVATE_trackLimboChange(e, r);
    ...
    __PRIVATE_logDebug(Xt, "Document no longer in limbo: " + r.key), e.au.removeReference(r.key, t);
...
function __PRIVATE_trackLimboChange(e, t) {
  ...
  e.ou.get(n) || e.su.has(r) || (__PRIVATE_logDebug(Xt, "New document in limbo: " + n), e.su.add(r), __PRIVATE_pumpEnqueuedLimboResolutions(e));
}
...
async function __PRIVATE_syncEngineEmitNewSnapsAndNotifyLocalStore(e, t, n) {
  ...
  r.ru.isEmpty() || (r.ru.forEach((e2, _) => {
    ...
    try {
      ...
      __PRIVATE_logDebug(Lt, "Failed to update sequence numbers: " + e3);
    }
    for (const e3 of t2) {
...
async function __PRIVATE_syncEngineHandleCredentialChange(e, t) {
  ...
  if (!n.currentUser.isEqual(t)) {
    ...
    function __PRIVATE_rejectOutstandingPendingWritesCallbacks(e3, t2) {
      e3.cu.forEach((e4) => {
        e4.forEach((e5) => {
          e5.reject(new FirestoreError(x.CANCELLED, t2));
        });
      }), e3.cu.clear();
...
var __PRIVATE_MultiTabOfflineComponentProvider = class extends __PRIVATE_IndexedDbOfflineComponentProvider {
  ...
  Au(e) {
    ...
    if (!__PRIVATE_WebStorageSharedClientState.C(t)) throw new FirestoreError(x.UNIMPLEMENTED, "IndexedDB persistence is only available on platforms that support LocalStorage.");
...
var Transaction$2 = class {
  ...
  async lookup(e) {
    if (this.ensureCommitNotCalled(), this.mutations.length > 0) throw this.lastTransactionError = new FirestoreError(x.INVALID_ARGUMENT, "Firestore transactions require all reads to be executed before all writes."), this.lastTransactionError;
  ...
  recordVersion(e) {
    ...
    if (n) {
      if (!t.isEqual(n))
        throw new FirestoreError(x.ABORTED, "Document version changed between two reads.");
  ...
  preconditionForUpdate(e) {
    ...
    if (!this.writtenDocs.has(e.toString()) && t) {
      if (t.isEqual(SnapshotVersion.min()))
        throw new FirestoreError(x.INVALID_ARGUMENT, "Can't update a document that doesn't exist.");
...
var FirestoreClient = class {
  constructor(e, t, n, r, i) {
    this.authCredentials = e, this.appCheckCredentials = t, this.asyncQueue = n, this.databaseInfo = r, this.user = User.UNAUTHENTICATED, this.clientId = __PRIVATE_AutoId.newId(), this.authCredentialListener = () => Promise.resolve(), this.appCheckCredentialListener = () => Promise.resolve(), this._uninitializedComponentsProvider = i, this.authCredentials.start(n, async (e2) => {
      __PRIVATE_logDebug(en, "Received user=", e2.uid), await this.authCredentialListener(e2), this.user = e2;
    }), this.appCheckCredentials.start(n, (e2) => (__PRIVATE_logDebug(en, "Received new app check token=", e2), this.appCheckCredentialListener(e2, this.user)));
  }
  get configuration() {
    return {
...
async function __PRIVATE_setOfflineComponentProvider(e, t) {
  e.asyncQueue.verifyOperationInProgress(), __PRIVATE_logDebug(en, "Initializing OfflineComponentProvider");
...
async function __PRIVATE_setOnlineComponentProvider(e, t) {
  ...
  __PRIVATE_logDebug(en, "Initializing OnlineComponentProvider"), await t.initialize(n, e.configuration), // The CredentialChangeListener of the online component provider takes
...
async function __PRIVATE_ensureOfflineComponents(e) {
  if (!e._offlineComponents) if (e._uninitializedComponentsProvider) {
    __PRIVATE_logDebug(en, "Using user provided OfflineComponentProvider");
    try {
      await __PRIVATE_setOfflineComponentProvider(e, e._uninitializedComponentsProvider._offline);
    } catch (t) {
      const n = t;
      if (!function __PRIVATE_canFallbackFromIndexedDbError(e2) {
        return "FirebaseError" === e2.name ? e2.code === x.FAILED_PRECONDITION || e2.code === x.UNIMPLEMENTED : !("undefined" != typeof DOMException && e2 instanceof DOMException) || // When the browser is out of quota we could get either quota exceeded
  ...
  } else __PRIVATE_logDebug(en, "Using default OfflineComponentProvider"), await __PRIVATE_setOfflineComponentProvider(e, new __PRIVATE_LruGcMemoryOfflineComponentProvider(void 0));
  return e._offlineComponents;
}
async function __PRIVATE_ensureOnlineComponents(e) {
  return e._onlineComponents || (e._uninitializedComponentsProvider ? (__PRIVATE_logDebug(en, "Using user provided OnlineComponentProvider"), await __PRIVATE_setOnlineComponentProvider(e, e._uninitializedComponentsProvider._online)) : (__PRIVATE_logDebug(en, "Using default OnlineComponentProvider"), await __PRIVATE_setOnlineComponentProvider(e, new OnlineComponentProvider()))), e._onlineComponents;
}
...
function __PRIVATE_firestoreClientGetDocumentFromLocalCache(e, t) {
  ...
  return e.asyncQueue.enqueueAndForget(async () => async function __PRIVATE_readDocumentFromCache(e2, t2, n2) {
    try {
      ...
      r.isFoundDocument() ? n2.resolve(r) : r.isNoDocument() ? n2.resolve(null) : n2.reject(new FirestoreError(x.UNAVAILABLE, "Failed to get document from cache. (However, this document may exist on the server. Run again without setting 'source' in the GetOptions to attempt to retrieve the document from the server.)"));
    } catch (e3) {
    ...
    const s = new __PRIVATE_AsyncObserver({
      next: (_) => {
        ...
        !a && _.fromCache ? (
          ...
          i.reject(new FirestoreError(x.UNAVAILABLE, "Failed to get document because the client is offline."))
        ) : a && _.fromCache && r2 && "server" === r2.source ? i.reject(new FirestoreError(x.UNAVAILABLE, 'Failed to get document from server. (However, this document does exist in the local cache. Run again without setting source to "server" to retrieve the cached document.)')) : i.resolve(_);
      },
...
function __PRIVATE_validateNonEmptyArgument(e, t, n) {
  if (!n) throw new FirestoreError(x.INVALID_ARGUMENT, `Function ${e}() cannot be called with an empty ${t}.`);
}
function __PRIVATE_validateIsNotUsedTogether(e, t, n, r) {
  if (true === t && true === r) throw new FirestoreError(x.INVALID_ARGUMENT, `${e} and ${n} cannot be used together.`);
}
function __PRIVATE_validateDocumentPath(e) {
  if (!DocumentKey.isDocumentKey(e)) throw new FirestoreError(x.INVALID_ARGUMENT, `Invalid document reference. Document references must have an even number of segments, but ${e} has ${e.length}.`);
}
function __PRIVATE_validateCollectionPath(e) {
  if (DocumentKey.isDocumentKey(e)) throw new FirestoreError(x.INVALID_ARGUMENT, `Invalid collection reference. Collection references must have an odd number of segments, but ${e} has ${e.length}.`);
}
...
function __PRIVATE_cast(e, t) {
  ...
  (e = e._delegate), !(e instanceof t)) {
    if (t.name === e.constructor.name) throw new FirestoreError(x.INVALID_ARGUMENT, "Type does not match the expected instance. Did you pass a reference from a different Firestore SDK?");
    {
      const n = __PRIVATE_valueDescription(e);
      throw new FirestoreError(x.INVALID_ARGUMENT, `Expected type '${t.name}', but it was: ${n}`);
    }
  }
  return e;
}
function __PRIVATE_validatePositiveNumber(e, t) {
  if (t <= 0) throw new FirestoreError(x.INVALID_ARGUMENT, `Function ${e}() requires a positive number, but it was: ${t}.`);
}
...
var FirestoreSettingsImpl = class {
  constructor(e) {
    var t, n;
    if (void 0 === e.host) {
      if (void 0 !== e.ssl) throw new FirestoreError(x.INVALID_ARGUMENT, "Can't provide ssl option if host option is not set");
      this.host = nn, this.ssl = rn;
    } else this.host = e.host, this.ssl = null !== (t = e.ssl) && void 0 !== t ? t : rn;
    if (this.isUsingEmulator = void 0 !== e.emulatorOptions, this.credentials = e.credentials, this.ignoreUndefinedProperties = !!e.ignoreUndefinedProperties, this.localCache = e.localCache, void 0 === e.cacheSizeBytes) this.cacheSizeBytes = vt;
    else {
      if (-1 !== e.cacheSizeBytes && e.cacheSizeBytes < Ft) throw new FirestoreError(x.INVALID_ARGUMENT, "cacheSizeBytes must be at least 1048576");
    ...
    __PRIVATE_validateIsNotUsedTogether("experimentalForceLongPolling", e.experimentalForceLongPolling, "experimentalAutoDetectLongPolling", e.experimentalAutoDetectLongPolling), this.experimentalForceLongPolling = !!e.experimentalForceLongPolling, this.experimentalForceLongPolling ? this.experimentalAutoDetectLongPolling = false : void 0 === e.experimentalAutoDetectLongPolling ? this.experimentalAutoDetectLongPolling = true : (
      ...
      if (void 0 !== e2.timeoutSeconds) {
        if (isNaN(e2.timeoutSeconds)) throw new FirestoreError(x.INVALID_ARGUMENT, `invalid long polling timeout: ${e2.timeoutSeconds} (must not be NaN)`);
        if (e2.timeoutSeconds < 5) throw new FirestoreError(x.INVALID_ARGUMENT, `invalid long polling timeout: ${e2.timeoutSeconds} (minimum allowed value is 5)`);
        if (e2.timeoutSeconds > 30) throw new FirestoreError(x.INVALID_ARGUMENT, `invalid long polling timeout: ${e2.timeoutSeconds} (maximum allowed value is 30)`);
      }
...
var Firestore$1 = class {
   ...
   */
  get app() {
    if (!this._app) throw new FirestoreError(x.FAILED_PRECONDITION, "Firestore was not initialized using the Firebase SDK. 'app' is not available");
    return this._app;
  }
  get _initialized() {
  ...
  _setSettings(e) {
    if (this._settingsFrozen) throw new FirestoreError(x.FAILED_PRECONDITION, "Firestore has already been started and its settings can no longer be changed. You can only modify settings before calling any other methods on a Firestore object.");
    this._settings = new FirestoreSettingsImpl(e), this._emulatorOptions = e.emulatorOptions || {}, void 0 !== e.credentials && (this._authCredentials = function __PRIVATE_makeAuthCredentialsProvider(e2) {
      ...
      switch (e2.type) {
        ...
        case "provider":
          return e2.client;
        default:
          throw new FirestoreError(x.INVALID_ARGUMENT, "makeAuthCredentialsProvider failed due to invalid credential type");
      }
    }(e.credentials));
  }
  _getSettings() {
  ...
  _terminate() {
    return function __PRIVATE_removeComponents(e) {
      const t = tn.get(e);
      t && (__PRIVATE_logDebug("ComponentProvider", "Removing Datastore"), tn.delete(e), t.terminate());
...
function connectFirestoreEmulator(e, t, n, r = {}) {
  ...
  if (!deepEqual(u, _) && (e._setSettings(u), r.mockUserToken)) {
    ...
    if ("string" == typeof r.mockUserToken) t2 = r.mockUserToken, n2 = User.MOCK_USER;
      ...
      if (!s2) throw new FirestoreError(x.INVALID_ARGUMENT, "mockUserToken must contain 'sub' or 'user_id' field!");
      n2 = new User(s2);
    }
...
function collection(e, t, ...n) {
  ...
  {
    if (!(e instanceof DocumentReference || e instanceof CollectionReference)) throw new FirestoreError(x.INVALID_ARGUMENT, "Expected first argument to collection() to be a CollectionReference, a DocumentReference or FirebaseFirestore");
...
function collectionGroup(e, t) {
  if (e = __PRIVATE_cast(e, Firestore$1), __PRIVATE_validateNonEmptyArgument("collectionGroup", "collection id", t), t.indexOf("/") >= 0) throw new FirestoreError(x.INVALID_ARGUMENT, `Invalid collection ID '${t}' passed to function collectionGroup(). Collection IDs must not contain '/'.`);
  return new Query(
    e,
    /* converter= */
    null,
...
var __PRIVATE_AsyncQueueImpl = class {
  constructor(e = Promise.resolve()) {
    ...
    this.ju = () => {
      ...
      e2 && __PRIVATE_logDebug(sn, "Visibility state changed to " + e2.visibilityState), this.y_.A_();
    }, this.Hu = e;
    const t = getDocument();
...
function initializeFirestore(e, t, n) {
  ...
  if (r.isInitialized(n)) {
    ...
    throw new FirestoreError(x.FAILED_PRECONDITION, "initializeFirestore() has already been called with different options. To avoid this error, call initializeFirestore() with the same options as when it was originally called, or call getFirestore() to return the already initialized instance.");
  }
...
function getFirestore(e, n) {
  const r = "object" == typeof e ? e : getApp(), i = "string" == typeof e ? e : n || ot, s = _getProvider(r, "firestore").getImmediate({
    identifier: i
  });
  if (!s._initialized) {
...
function ensureFirestoreConfigured(e) {
  if (e._terminated) throw new FirestoreError(x.FAILED_PRECONDITION, "The client has already been terminated.");
...
function __PRIVATE_setPersistenceProviders(e, t, n) {
  if ((e = __PRIVATE_cast(e, Firestore))._firestoreClient || e._terminated) throw new FirestoreError(x.FAILED_PRECONDITION, "Firestore has already been started and persistence can no longer be enabled. You can only enable persistence before calling any other methods on a Firestore object.");
  if (e._componentsProvider || e._getSettings().localCache) throw new FirestoreError(x.FAILED_PRECONDITION, "SDK cache is already specified.");
  e._componentsProvider = {
    _online: t,
    _offline: n
...
function clearIndexedDbPersistence(e) {
  if (e._initialized && !e._terminated) throw new FirestoreError(x.FAILED_PRECONDITION, "Persistence can only be cleared before a Firestore instance is initialized or after it is terminated.");
  const t = new __PRIVATE_Deferred();
...
var Bytes = class _Bytes {
  ...
  static fromBase64String(e) {
    try {
      ...
      throw new FirestoreError(x.INVALID_ARGUMENT, "Failed to construct data from Base64 string: " + e2);
    }
  }
  /**
...
var FieldPath = class {
   ...
   */
  constructor(...e) {
    for (let t = 0; t < e.length; ++t) if (0 === e[t].length) throw new FirestoreError(x.INVALID_ARGUMENT, "Invalid field name at argument $(i + 1). Field names must not be empty.");
...
var GeoPoint = class {
   ...
   */
  constructor(e, t) {
    if (!isFinite(e) || e < -90 || e > 90) throw new FirestoreError(x.INVALID_ARGUMENT, "Latitude must be a number between -90 and 90, but was: " + e);
    if (!isFinite(t) || t < -180 || t > 180) throw new FirestoreError(x.INVALID_ARGUMENT, "Longitude must be a number between -180 and 180, but was: " + t);
...
var __PRIVATE_ParseContextImpl = class ___PRIVATE_ParseContextImpl {
  ...
  lc(e) {
    ...
    if (__PRIVATE_isWrite(this.oc) && _n.test(e)) throw this.Tc('Document fields cannot begin and end with "__"');
  }
};
...
function __PRIVATE_parseSetData(e, t, n, r, i, s = {}) {
  ...
  if (s.merge) a = new FieldMask(o.fieldMask), u = o.fieldTransforms;
    ...
    for (const r2 of s.mergeFields) {
      ...
      if (!o.contains(i2)) throw new FirestoreError(x.INVALID_ARGUMENT, `Field '${i2}' is specified in your field mask but missing from your input data.`);
...
function __PRIVATE_parseUpdateVarargs(e, t, n, r, i, s) {
  ...
  if (s.length % 2 != 0) throw new FirestoreError(x.INVALID_ARGUMENT, `Function ${t}() needs to be called with an even number of arguments that alternate between field names and values.`);
...
function __PRIVATE_createError(e, t, n, r, i) {
  ...
  let a = "";
  return (s || o) && (a += " (found", s && (a += ` in field ${r}`), o && (a += ` in document ${i}`), a += ")"), new FirestoreError(x.INVALID_ARGUMENT, _ + e + a);
}
...
function __PRIVATE_validateHasExplicitOrderByForLimitToLast(e) {
  if ("L" === e.limitType && 0 === e.explicitOrderBy.length) throw new FirestoreError(x.UNIMPLEMENTED, "limitToLast() queries require specifying at least one orderBy() clause");
}
...
function query(e, t, ...n) {
  ...
  t instanceof AppliableConstraint && r.push(t), r = r.concat(n), function __PRIVATE_validateQueryConstraintArray(e2) {
    ...
    if (t2 > 1 || t2 > 0 && n2 > 0) throw new FirestoreError(x.INVALID_ARGUMENT, "InvalidQuery. When using composite filters, you cannot use more than one filter at the top level. Consider nesting the multiple filters within an `and(...)` statement. For example: change `query(query, where(...), or(...))` to `query(query, and(where(...), or(...)))`.");
  }(r);
...
var QueryFieldFilterConstraint = class _QueryFieldFilterConstraint extends QueryConstraint {
  ...
  _parse(e) {
    const t = __PRIVATE_newUserDataReader(e.firestore), n = function __PRIVATE_newQueryFilter(e2, t2, n2, r, i, s, o) {
      let _;
      if (i.isKeyField()) {
        if ("array-contains" === s || "array-contains-any" === s) throw new FirestoreError(x.INVALID_ARGUMENT, `Invalid Query. You can't perform '${s}' queries on documentId().`);
        if ("in" === s || "not-in" === s) {
...
var QueryOrderByConstraint = class _QueryOrderByConstraint extends QueryConstraint {
  ...
  _apply(e) {
    const t = function __PRIVATE_newQueryOrderBy(e2, t2, n) {
      if (null !== e2.startAt) throw new FirestoreError(x.INVALID_ARGUMENT, "Invalid query. You must not call startAt() or startAfter() before calling orderBy().");
      if (null !== e2.endAt) throw new FirestoreError(x.INVALID_ARGUMENT, "Invalid query. You must not call endAt() or endBefore() before calling orderBy().");
...
function __PRIVATE_newQueryBoundFromDocOrFields(e, t, n, r) {
  if (n[0] = getModularInstance(n[0]), n[0] instanceof DocumentSnapshot$1) return function __PRIVATE_newQueryBoundFromDocument(e2, t2, n2, r2, i) {
    if (!r2) throw new FirestoreError(x.NOT_FOUND, `Can't use a DocumentSnapshot that doesn't exist for ${n2}().`);
    const s = [];
    for (const n3 of __PRIVATE_queryNormalizedOrderBy(e2)) if (n3.field.isKeyField()) s.push(__PRIVATE_refValue(t2, r2.key));
      ...
      if (__PRIVATE_isServerTimestamp(e3)) throw new FirestoreError(x.INVALID_ARGUMENT, 'Invalid query. You are trying to start or end a query using a document for which the field "' + n3.field + '" is an uncommitted server timestamp. (Since the value of this field is unknown, you cannot start/end a query with it.)');
      if (null === e3) {
        const e4 = n3.field.canonicalString();
        throw new FirestoreError(x.INVALID_ARGUMENT, `Invalid query. You are trying to start or end a query using a document for which the field '${e4}' (used as the orderBy) does not exist.`);
      }
      s.push(e3);
    }
  ...
  }(e._query, e.firestore._databaseId, t, n[0]._document, r);
  {
    ...
    return function __PRIVATE_newQueryBoundFromFields(e2, t2, n2, r2, i2, s) {
      ...
      if (i2.length > o.length) throw new FirestoreError(x.INVALID_ARGUMENT, `Too many arguments provided to ${r2}(). The number of arguments must be less than or equal to the number of orderBy() clauses`);
      const _ = [];
      for (let s2 = 0; s2 < i2.length; s2++) {
        const a = i2[s2];
        if (o[s2].field.isKeyField()) {
          if ("string" != typeof a) throw new FirestoreError(x.INVALID_ARGUMENT, `Invalid query. Expected a string for document ID in ${r2}(), but got a ${typeof a}`);
          if (!__PRIVATE_isCollectionGroupQuery(e2) && -1 !== a.indexOf("/")) throw new FirestoreError(x.INVALID_ARGUMENT, `Invalid query. When querying a collection and ordering by documentId(), the value passed to ${r2}() must be a plain document ID, but '${a}' contains a slash.`);
          const n3 = e2.path.child(ResourcePath.fromString(a));
          if (!DocumentKey.isDocumentKey(n3)) throw new FirestoreError(x.INVALID_ARGUMENT, `Invalid query. When querying a collection group and ordering by documentId(), the value passed to ${r2}() must result in a valid document path, but '${n3}' is not because it contains an odd number of segments.`);
...
function __PRIVATE_parseDocumentIdValue(e, t, n) {
  if ("string" == typeof (n = getModularInstance(n))) {
    if ("" === n) throw new FirestoreError(x.INVALID_ARGUMENT, "Invalid query. When querying with documentId(), you must provide a valid document ID, but it was an empty string.");
    if (!__PRIVATE_isCollectionGroupQuery(t) && -1 !== n.indexOf("/")) throw new FirestoreError(x.INVALID_ARGUMENT, `Invalid query. When querying a collection by documentId(), you must provide a plain document ID, but '${n}' contains a '/' character.`);
    const r = t.path.child(ResourcePath.fromString(n));
    if (!DocumentKey.isDocumentKey(r)) throw new FirestoreError(x.INVALID_ARGUMENT, `Invalid query. When querying a collection group by documentId(), the value provided must result in a valid document path, but '${r}' is not because it has an odd number of segments (${r.length}).`);
    return __PRIVATE_refValue(e, new DocumentKey(r));
  }
  if (n instanceof DocumentReference) return __PRIVATE_refValue(e, n._key);
  throw new FirestoreError(x.INVALID_ARGUMENT, `Invalid query. When querying with documentId(), you must provide a valid string or a DocumentReference, but it was: ${__PRIVATE_valueDescription(n)}.`);
}
function __PRIVATE_validateDisjunctiveFilterElements(e, t) {
  if (!Array.isArray(e) || 0 === e.length) throw new FirestoreError(x.INVALID_ARGUMENT, `Invalid Query. A non-empty array is required for '${t.toString()}' filters.`);
}
function __PRIVATE_validateNewFieldFilter(e, t) {
  ...
  if (null !== n)
    throw n === t.op ? new FirestoreError(x.INVALID_ARGUMENT, `Invalid query. You cannot use more than one '${t.op.toString()}' filter.`) : new FirestoreError(x.INVALID_ARGUMENT, `Invalid query. You cannot use '${t.op.toString()}' filters with '${n.toString()}' filters.`);
}
function __PRIVATE_validateQueryFilterConstraint(e, t) {
  if (!(t instanceof QueryFieldFilterConstraint || t instanceof QueryCompositeFilterConstraint)) throw new FirestoreError(x.INVALID_ARGUMENT, `Function ${e}() requires AppliableConstraints created with a call to 'where(...)', 'or(...)', or 'and(...)'.`);
}
var AbstractUserDataWriter = class {
  ...
  convertVectorValue(e) {
    var t, n, r;
    const i = null === (r = null === (n = null === (t = e.fields) || void 0 === t ? void 0 : t[lt].arrayValue) || void 0 === n ? void 0 : n.values) || void 0 === r ? void 0 : r.map((e2) => __PRIVATE_normalizeNumber(e2.doubleValue));
...
var QuerySnapshot = class {
  ...
  docChanges(e = {}) {
    ...
    if (t && this._snapshot.excludesMetadataChanges) throw new FirestoreError(x.INVALID_ARGUMENT, "To include metadata changes with your document changes, you must also pass { includeMetadataChanges:true } to onSnapshot().");
...
var WriteBatch = class {
  ...
  _verifyNotCommitted() {
    if (this._committed) throw new FirestoreError(x.FAILED_PRECONDITION, "A write batch can no longer be used after commit() has been called.");
  }
};
function __PRIVATE_validateReference(e, t) {
  if ((e = getModularInstance(e)).firestore !== t) throw new FirestoreError(x.INVALID_ARGUMENT, "Provided document reference is from a different Firestore instance.");
  return e;
}
...
function runTransaction(e, t, n) {
  ...
  const r = Object.assign(Object.assign({}, un), n);
  !function __PRIVATE_validateTransactionOptions(e2) {
    if (e2.maxAttempts < 1) throw new FirestoreError(x.INVALID_ARGUMENT, "Max attempts must be at least 1");
  }(r);
  ...
  const r = function __PRIVATE_parseIndexes(e2) {
    const t2 = "string" == typeof e2 ? function __PRIVATE_tryParseJson(e3) {
      try {
        ...
        throw new FirestoreError(x.INVALID_ARGUMENT, "Failed to parse JSON: " + (null == e4 ? void 0 : e4.message));
      }
    }(e2) : e2, n2 = [];
...
function __PRIVATE_tryGetString(e, t) {
  if ("string" != typeof e[t]) throw new FirestoreError(x.INVALID_ARGUMENT, "Missing string value for: " + t);
  return e[t];
}
...
var __PRIVATE_TestingHooksSpiImpl = class ___PRIVATE_TestingHooksSpiImpl {
  ...
  static get instance() {
    return ln || (ln = new ___PRIVATE_TestingHooksSpiImpl(), function __PRIVATE_setTestingHooksSpi(e) {
      if (mt) throw new Error("a TestingHooksSpi instance is already set");
      mt = e;
    }(ln)), ln;
  }
  ht(e) {
...
!function __PRIVATE_registerFirestore(e, t = true) {
  !function __PRIVATE_setSDKVersion(e2) {
    ...
    const i = e2.getProvider("app").getImmediate(), s = new Firestore(new __PRIVATE_FirebaseAuthCredentialsProvider(e2.getProvider("auth-internal")), new __PRIVATE_FirebaseAppCheckTokenProvider(i, e2.getProvider("app-check-internal")), function __PRIVATE_databaseIdFromApp(e3, t2) {
      if (!Object.prototype.hasOwnProperty.apply(e3.options, ["projectId"])) throw new FirestoreError(x.INVALID_ARGUMENT, '"projectId" not provided in firebase.initializeApp.');
      return new DatabaseId(e3.options.projectId, t2);
    }(i, n), i);
    return r = Object.assign({
      useFetchStreams: t
    }, r), s._setSettings(r), s;
  }, "PUBLIC").setMultipleInstances(true)), registerVersion(v, C, e), // BUILD_TARGET will be replaced by values like esm2017, cjs2017, etc during the compilation
  registerVersion(v, C, "esm2017");
}();
export {
